...
export function BusinessHero({ member }: BusinessHeroProps) {
  ...
  const mobileStats = [
    ...
    },
    {
      ...
      icon: (
        <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4 text-white">
          <path d="M3 17l6-6 4 4 8-8M21 7v6M21 7h-6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      ),
    },
    {
      ...
      icon: (
        <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4 text-white">
          <path d="M20 12v9H4v-9M2 7h20v5H2V7zM12 22V7M12 7H7.5a2.5 2.5 0 010-5C11 2 12 7 12 7zM12 7h4.5a2.5 2.5 0 000-5C9 2 12 7 12 7z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      ),
    },
  ];
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ background: "linear-gradient(160deg, rgba(193,20,43,1) 0%, rgba(80,6,15,1) 100%)" }}
      >
        {/* Top: logo + name */}
        ...
        <div className="space-y-3 mb-5">
          {mobileStats.map((stat) => (
            <div key={stat.label} className="flex items-center gap-3 bg-white/10 rounded-xl p-3">
              <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center flex-shrink-0">
                {stat.icon}
              </div>
              <div className="flex-1 min-w-0">
                <p className="!text-white font-bold text-[18px] leading-tight">{stat.value}</p>
                <p className="!text-white/70 text-[12px] mt-0.5">{stat.label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-3">
          <button
            onClick={() => router.push("/dashboard/profile")}
            className="w-full flex items-center justify-center h-11 gap-2 rounded-xl text-[14px] font-semibold"
            style={{ border: "1px solid #F8C600", color: "#F8C600", background: "transparent" }}
          >
            View My Profile
            <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4">
              <path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6M15 3h6m0 0v6m0-6L10 14" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          ...
          <button
            onClick={handleCreateOffer}
            disabled={checking}
            className="w-full flex text-white items-center justify-center h-11 gap-2 rounded-xl text-[14px] font-semibold disabled:opacity-70"
            style={{ background: "#C31526" }}
          >
            {checking ? "Checking..." : "Create new offer"}
            <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4">
              <path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
