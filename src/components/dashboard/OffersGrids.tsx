...
import { getAllActiveOffers, recordOfferView, toggleOfferSave } from "@/lib/api";
...
export function OffersGrid({ selectedCategory, onClearCategory, member }: OffersGridProps) {
  ...
  useEffect(() => {
    setLoading(true);
    getAllActiveOffers(getToken(), selectedCategory ?? undefined)
      .then((res) => {
          ...
          return (
            ...
            >
              <div className={`relative h-36 bg-gradient-to-br ${bgColors[i % bgColors.length]} flex items-center justify-center overflow-hidden`}>
                ...
                >
                  <svg viewBox="0 0 24 24" fill={isSaved ? "currentColor" : "none"} className={`w-3.5 h-3.5 ${isSaved ? "text-primary" : "text-muted"}`}>
